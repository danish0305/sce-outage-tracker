"""Step 1 - Discover Glendale Water & Power outage endpoint (DataVoice / outageentry.com)."""
import json, re, pathlib, traceback
from playwright.sync_api import sync_playwright

URL = "https://www.outageentry.com/Outages/outage.php?Client=glendale"
ZIPS = ["91206", "91201"]   # Glendale
...
NOISE = re.compile(r"(google|gtm|gtag|analytics|doubleclick|facebook|adobe|demdex|omtrdc|"
    r"quantum|optimizely|hotjar|newrelic|cookielaw|onetrust|cookiebot|tealium|"
    r"\.png|\.jpg|\.jpeg|\.gif|\.svg|\.woff|\.woff2|\.ttf|\.css|\.ico|fonts\.|/fonts/)", re.I)
INTEREST = re.compile(r"(kubra|arcgis|esri|featureserver|outage|/api/|geojson|query|\.php|"
    r"\.json|\.ashx|\.asmx|xml|circuit|incident|data|marker|kml)", re.I)
...
def main():
    with sync_playwright() as p:
        ...
        # pan/zoom any map surface
        for s in ["canvas",".leaflet-container",".gm-style","#map","div[class*='map' i]","iframe"]:
            try:
        ...
        for z in ZIPS:
            ...
            for s in ["input[type='search']","input[placeholder*='zip' i]","input[placeholder*='address' i]","input[type='text']"]:
                try:
                    loc=page.locator(s).first
                    if loc.count() and loc.is_visible():
                        loc.click(); loc.fill(""); loc.type(z, delay=60); page.wait_for_timeout(700); loc.press("Enter")
        ...
        try:
            ...
            for i,f in enumerate(page.frames):
                try: (OUT/f"frame_{i}.html").write_text(f.content(), encoding="utf-8", errors="ignore")
